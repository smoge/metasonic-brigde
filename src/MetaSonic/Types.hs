...
MetaSonic.Bridge.IR
  First compilation pass. This is where the source vocabulary (UGen,
  Connection) is stripped and replaced with the compiler's vocabulary
  (NodeIR, InputConn) plus semantic annotations (Rate, Eff).
  lowerGraph ties together validation, sorting, lowering, and rate
  checking in one function. This is the module where the argument
  about surface syntax vs semantic syntax becomes concrete.

MetaSonic.Bridge.Compile
  Read formRegions (region formation), then compileRuntimeGraph (the
  decisive NodeID → NodeIndex transformation). The Region and
  RegionGraph types are where scheduling logic lives. See Note [Region
  formation] and Note [Dense lowering] in MetaSonic.Compile.

MetaSonic.Bridge.FFI
  loadRuntimeGraph is the marshaling function that walks the dense
  RuntimeGraph and emits FFI calls. After reading this, you know
  exactly what crosses to C++ and in what form.
...
1. Nominal identity matters. A NodeID is not a NodeIndex, even though
   both wrap Int. The Haskell type system enforces the distinction
   between symbolic identifiers (which exist only during compilation)
   and dense runtime positions (which survive into the C++ runtime).
   Collapsing them would allow the kind of symbolic- runtime confusion
   that MetaSonic exists to prevent.

2. Semantic annotations are compilation artifacts. The Rate and Eff
   types exist because time and resource access are part of the
   compilation problem, not runtime concerns. Faust's semantic typing
   dimensions — signal nature, interval, computation time, speed,
   parallelizability — motivate a richer typed IR in which these
   distinctions become explicit. MetaSonic extends Faust's list with
   resource effects, which become essential for correct parallel
   scheduling.

3. The vocabulary is shared, but the flow is one-directional. Types
   defined here are imported by every module from Source through FFI,
   but information flows strictly forward through the pipeline. No
   downstream module writes back into an upstream representation. This
   is the module-level expression of the staged architecture.
-}
...
NodeID, NodeIndex, PortIndex, and ControlIndex are represented as
distinct newtypes, preserving nominal distinctions between otherwise
integer-like identifiers.

Symbolic identifiers (NodeID) exist on the Haskell side only. A NodeID
names a node in the source graph; it is meaningful during
construction, validation, and ordering. It does not survive into the
runtime.

The decisive transformation in the compiler (compileRuntimeGraph in
MetaSonic.Compile) replaces every NodeID with a dense NodeIndex, after
which symbolic identity is erased. See Note [Dense lowering] in
MetaSonic.Compile.

Dense identifiers (NodeIndex, PortIndex, ControlIndex) cross the FFI
boundary. On the C++ side, the same nominal distinctions are
maintained (NodeIndex, PortIndex, ControlIndex are separate structs in
rt_graph.cpp), ensuring that the Haskell compiler and the C++ runtime
agree on what each integer means — by convention, not by a shared type
definition, but the convention is enforced by identical newtype/struct
...
data NodeKind
  ...
  | KBusIn
  | KBusInDelayed
    -- ^ Read the *previous* block's contents of an audio bus. Sibling
    -- of 'KBusIn' but reads from the frozen prior-block snapshot in
    -- the runtime's double-buffered bus pool, so feedback loops can
    -- be expressed as @BusInDelayed → ... → BusOut@ without closing
    -- an E_r cycle. See Note [Effect-induced edges (E_r)] in
    -- "MetaSonic.Bridge.Validate" for why the schedulable graph drops
    -- 'BusReadDelayed' from E_r, and Note [Bus pool double-buffering]
    -- in @tinysynth/rt_graph.cpp@ for the runtime swap.
  | KDelay
    ...
    -- @q::delay@ (a 'fractional_ring_buffer' with linear
    -- interpolation), so the delay time can be sub-sample and
    -- modulated at audio rate. See Note [Per-node delay state] in
  ...
  | KSmooth
    ...
    -- smoothed sample-by-sample output. See Note [Per-node smooth
  ...
  | KTriOsc
    ...
    -- @[freq, phase]@. The triangle's stateless waveshape pairs well
    -- with FM via the freq input.
  | KHPF
    -- ^ High-pass biquad (wraps Q's @q::highpass@). Same input shape
    -- as 'KLPF': @[signal, cutoff, q]@. cutoff in Hz, q is the
    -- resonance. The filter reads cutoff and q once at the start of
    -- each block, matching 'KLPF'. Put 'KSmooth' before the cutoff
    -- input when MIDI or UI changes should glide from block to block
    -- instead of jumping; true within-block sweeps would need a
    -- sample-accurate filter path.
  | KBPF
    -- ^ Band-pass biquad (wraps Q's @q::bandpass_cpg@, the
    -- constant-peak-gain variant favored for music). Same input shape
    -- as 'KLPF': @[signal, cutoff, q]@. q controls bandwidth (higher
    -- q = narrower band). Cutoff and q are read once per block, like
    -- 'KHPF' and 'KLPF'.
  | KNotch
...
'KindSpec' centralises the *kind-level* facts about each 'NodeKind':
ABI tag, rate, audio/control arities, and label. Adding a new
'NodeKind' is one row here plus a constructor in 'NodeKind', a
constructor in 'UGen', a row in 'ugenView' (Source), and a builder.

The 'ksAudioArity' and 'ksControlArity' fields are cross-checked
against 'ugenView' by a property test, so drift between the table and
the constructor shape is caught at test time rather than in the
runtime.

Effects are deliberately *not* stored here. 'BusOut n' / 'BusIn n'
carry a bus number in a constructor field, so their 'Eff' annotation
('BusWrite n' / 'BusRead n') depends on per-instance data that the
kind-level table cannot represent. Putting '[Pure]' in a kind-level
effects column for those kinds would be a lie that defeats the
scheduling pass — the whole point of the 'Eff' machinery is that
'BusRead' / 'BusWrite' annotations drive effect-induced edges in
'MetaSonic.Bridge.Validate'.

The corresponding per-UGen function 'MetaSonic.Bridge.IR.inferEff' is
the single source of truth for effects; it returns '[Pure]' for the
kinds where that is honest and overrides for the kinds whose effects
depend on constructor fields. See Note [Effect-induced edges (E_r)] in
"MetaSonic.Bridge.Validate" for how those annotations drive
scheduling.

'ksRate' is interpreted as the kind's *intrinsic minimum* rate — a
floor. The actual rate of a node is the join (max) of this floor and
the rates of its inputs, computed by
'MetaSonic.Bridge.IR.propagateRates' as a post-pass on the lowered IR.
So a 'Gain' (floor 'CompileRate') fed by two 'Param' literals stays at
'CompileRate'; the same 'Gain' fed by a 'SinOsc' (floor 'SampleRate')
is lifted to 'SampleRate' by propagation. A 'SinOsc' is *always*
'SampleRate' regardless of inputs because its floor is 'SampleRate'
and the join can only raise the rate, never lower it. See Note [Rate
inference vs rate propagation] in "MetaSonic.Bridge.IR" for the
algorithm.
...
all-'Param' nodes can collapse to a one-time evaluation, while a
region fed by an oscillator necessarily runs sample-by-sample.
-}
...
data KindSpec = KindSpec
  ...
  , ksRate         :: !Rate
    -- ^ Kind-level *minimum* rate. The actual rate of a node is @max
    -- ksRate (max of input rates)@, computed by
    -- 'MetaSonic.Bridge.IR.propagateRates'. Stateful or
    -- sample-producing kinds set this to 'SampleRate'; stateless
    -- transforms and consumers set it to 'CompileRate' so they
    -- inherit from inputs. See Note [Per-kind metadata table] and
    -- Note [Rate inference vs rate propagation] in
    -- "MetaSonic.Bridge.IR".
  , ksAudioArity   :: !Int
    -- ^ Number of @Connection@ inputs the corresponding 'UGen'
    -- constructor carries.
  , ksControlArity :: !Int
    -- ^ Number of control slots the C++ kernel expects, in declared
    -- order. The mapping from inputs to controls is per-kind and is
    -- realized by 'MetaSonic.Bridge.Source.ugenView'; this field
    -- exists for cross-check only.
  , ksLabel        :: !String
...
--
-- Note: tag 4 was reserved for a generic biquad family but is
-- intentionally unallocated. The Haskell→C++ contract test (see
-- Spec.hs) iterates @[minBound..maxBound]@ via the derived
-- 'Enum'\/'Bounded' instances on 'NodeKind', so any new constructor
-- added here is automatically asserted against the C++
...
kindSpec = \case
  ...
  --
  --   * The biquad family (LPF, HPF, BPF, Notch) and Env are
  --     stateful: a block-rate biquad would alias and a block-rate
  --     envelope would miss gate transitions.
  --
  ...
  -- Bus routing: same-cycle BusOut/BusIn and one-block-delayed
  -- BusInDelayed. Effects are per-instance (BusWrite n / BusRead n /
  -- BusReadDelayed n) and live in 'inferEff', not here. The C++
  -- runtime stores all buses in one double-buffered pool
  -- (g.output_buses + g.output_buses_prev); the audio callback routes
  -- [0..N-1] to hardware regardless of which kind wrote them, so KOut
  -- and KBusOut are operationally identical and KBusIn reads from the
  -- live pool while KBusInDelayed reads from the previous-block
  -- snapshot.
  --
  -- See Note [Effect-induced edges (E_r)] in
  -- MetaSonic.Bridge.Validate and Note [Bus pool double-buffering] in
  -- tinysynth/rt_graph.cpp.
  KBusIn        -> KindSpec 11 SampleRate  0 1 "busIn"
  KBusInDelayed -> KindSpec 12 SampleRate  0 1 "busInDelayed"
  -- Delay line: per-node fractional ring buffer. Stateful (the buffer
  -- carries per-sample history) so the floor is SampleRate. 2 audio
  -- inputs: signal, delay-time-in-seconds. 2 controls:
  ...
  -- Smoother: per-node q::dynamic_smoother. 1 audio input (the value
  -- to smooth, often a Param the producer thread updates via the
  -- realtime ABI), 2 controls [base_freq_hz, target_default].
  ...
  -- the maximum rate of their inputs. A Gain with two Param inputs
  -- stays at CompileRate (no-op multiplication of constants); the
  -- same Gain fed by a SinOsc lifts to SampleRate by propagation.
  --
  --   * Gain, Add: stateless arithmetic.
  --
  --   * LPF would also be stateless except for its biquad delay
  --     state, which is why it sits in the stateful group above.
  --
...
discipline, a rate discipline, and potentially resource effects and
synchronization constraints.

For comparison:

Faust's compiler distinguishes five semantic typing dimensions: signal
nature, interval of values, computation time, speed (constant,
block/UI-rate, or sample-rate), and parallelizability.

MetaSonic adopts the speed dimension and uses it directly as the Rate
type.
...
A higher-rate node may freely read from a lower-rate node (the value
is simply held constant over the faster time scale — sample-and-hold).
A lower-rate node reading from a higher-rate node requires explicit
downsampling; this is a compilation error unless a conversion node is
inserted.

Rate annotations influence three downstream passes:

  1. Validation — checkRateEdges in MetaSonic.IR rejects edges where a
     lower-rate node reads from a higher-rate node without explicit
     conversion.

  2. Region formation — nodes must share a compatible rate to belong
     to the same region.
...
value is known statically and is trivially sample-and-held). A
region's 'regRate' is the maximum of its members' rates. 'BlockRate'
into 'SampleRate' would need an explicit sample-and-hold boundary in
the runtime and is deferred until a kind actually produces 'BlockRate'
...
'NodeKind'. It is /per-port/ and /destination-side/: a separate axis
from 'Rate', which is the /per-node/ /producer-side/ rate the IR
computes via 'propagateRates'.
...
'PortInitOnly' destination port writes 'nframes' samples that the
consumer will discard. That edge is where block-rate region
...
sample-accurate classification is pinned by a comment to the specific
runtime kernel that block-latches or init-reads the port.

This metadata is /descriptive/. Nothing in the runtime consumes it; it
exists so '--fusion-survey' can measure how many edges in real graphs
would benefit from a block-rate execution path before any such path is
implemented.
-}
...
data PortConsumptionRate
  = PortSampleAccurate
    ...
    -- side fall here.
  | PortBlockLatched
    -- ^ Read only at sample 0 of each block. The destination discards
    -- @nframes - 1@ samples of the source per block. Currently: the
    -- @freq@ and @q@ ports of the biquad family ('KLPF', 'KHPF',
    -- 'KBPF', 'KNotch'). The C++ kernels explicitly take only
    -- @freq_in[0]@ and @q_in[0]@ before reconfiguring the filter once
    -- per block.
  | PortInitOnly
    -- ^ Read only at instance configuration / construction. Never
    -- sampled in the per-block audio loop. Currently no kind has a
    -- port classified this way for 'RFrom' edges — kept as a distinct
    -- policy because "consumed once at init" is semantically
    -- different from "never consumed at all" ('PortIgnored') and from
    -- "consumed at block rate" ('PortBlockLatched'). A future kind
    -- whose runtime kernel reads an audio-input port exactly once at
    -- configure time would land here.
  | PortIgnored
    -- ^ The runtime silently discards 'RFrom' edges to this port.
    -- Currently: the @phase@ port (port 1) of the oscillator family
    -- ('KSinOsc', 'KSawOsc', 'KTriOsc', 'KPulseOsc').
    -- @process_sinosc@ / @process_sawosc@ / @process_triosc@ /
    -- @process_pulse_osc@ never call 'resolve_input' on port 1 inside
    -- the audio loop; the initial phase is taken from 'rnControls[1]'
    -- at instance construction, so a wired 'RFrom' source is dropped
    -- without affecting output.
    --
    -- Excluded from the §4.D.2 opportunity count: an ignored consumer
    -- represents no work the runtime is doing, so block-rate
    -- scheduling can't save anything by demoting the producer.
    -- (Dead-input elimination is a separate optimization concern.)
  deriving stock    (Eq, Ord, Show, Generic, Enum, Bounded)
  deriving anyclass (NFData)

-- | Bundle a port's consumption rate with a human-readable name so
-- the §4.D.2 survey can produce legible "kind.port" examples without
...
data PortInfo = PortInfo
  ...
  , piName   :: !String
    ...
    -- @"sig"@, @"amount"@). Matches the source-DSL builder argument
    -- name where one exists.
  } deriving stock    (Eq, Show, Generic)
    deriving anyclass (NFData)

-- | Per-kind, per-port consumption policy. Returns 'Nothing' for
-- ports outside the kind's audio-input range; the audio-input range
-- itself comes from 'kindSpec'\'s 'ksAudioArity' field, so this
-- function is total over the declared port range.
--
-- Cross-checked against 'kindSpec' / 'ugenView' by a property test in
-- @test/Spec.hs@: every kind's declared audio-input count must agree
-- with the highest port index 'portInfo' claims, and every port in
-- @[0 .. ksAudioArity - 1]@ must return 'Just'.
...
determine correct parallel execution. If two subgraphs read and write
shared buses or buffers, the linearized order of a sequential graph
may induce a semantic dependency not visible in the explicit graph
structure.
...
abstraction enabling Phase 2 feedback. In SuperCollider terms, BusRead
corresponds to 'In.ar' and BusReadDelayed corresponds to
'InFeedback.ar'.
-}
