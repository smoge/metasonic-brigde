...
-- Description : Phase 6.B.2a — pure OSC dispatch resolver.
--
-- Resolves a parsed 'OscMessage' against a 'ResolveState' (the
-- producer-side mapping table named in the §6.B design note —
-- the §5.4.C producer mapping problem made concrete). Returns
-- either a 'DispatchAction' the IO layer can hand to the
-- realtime queue, or a 'DispatchIssue' the IO layer logs and
-- drops.
--
-- v1 grammar: control writes only.
--
-- > /<voice-key>/<node-tag>/<slot>      ,f|,i  <value>
--
-- Voice lifecycle and hot-swap addresses are deferred — see the
-- design note for the reason.
