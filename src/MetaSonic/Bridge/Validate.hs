...
--
-- Validation is the gate between graph construction and compilation:
-- a graph that passes validation can be compiled; a graph that fails
-- cannot. No invalid structure should ever reach the runtime.
--
-- See Note [Structural vs semantic well-formedness] for what this
-- module checks and what it deliberately leaves to later passes.
--
-- See Note [Topological sort as compilation target] for how the sort computed
-- here becomes the storage order of the C++ runtime.
...
This module enforces two structural well-formedness conditions:
  ...
  1. Referential integrity: every NodeID referenced by a Connection
     must exist in the graph. A dangling reference is a compilation
     error. Checked by checkDependencies.

  2. Acyclicity: the explicit dependency graph must be a DAG. A cycle
     is a compilation error. Checked by topoSort.

Structural validation is necessary but not sufficient for semantic
well-formedness. The following conditions are checked or will be
checked by later passes:

  - Rate mismatches requiring conversion nodes. Checked by
    checkRateEdges in MetaSonic.IR. See Note [Rate discipline] in
    MetaSonic.Types.

  - Implicit resource dependencies not represented by graph edges.
    Future: effect analysis using Eff annotations. See Note [Resource
    effects] in MetaSonic.Types.

  - Recursive definitions whose state semantics must be made explicit.
    Future: delay semantics. The current prototype requires strict
    acyclicity.

  - One-sided dependencies from analysis or recorder nodes. Future:
    satellite edge insertion, following SuperNova's design.

The separation is intentional: structural checks are cheap and catch
the most common errors early. Semantic checks require annotation
(rates, effects) that does not exist until after lowering in
MetaSonic.IR.
-}
...
The topological sort establishes execution order. After this pass,
every node has a fixed position in the evaluation sequence, and that
sequence respects all explicit data dependencies.

This list becomes the storage order of the dense runtime array in C++,
...
SuperNova identifies sequential linearized traversal as efficient for
fine-grained graphs in the sequential case. MetaSonic treats this not
as a convenient implementation shortcut but as the desired target of
compilation. The sort computed here is the origin of that
linearization.
...
The original monolithic code had a double-toposort bug: validateGraph
called topoSort internally to check for cycles (discarding the
result), then lowerGraph called topoSort again to get the execution
order. Three traversals for one result.

validateAndSort computes both checks in a single traversal:
checkDependencies verifies referential integrity, then topoSort
verifies acyclicity and produces the execution order. The result is
threaded forward to lowerGraph in MetaSonic.IR, which uses it directly
without re-sorting.
-}
...
Why this pass exists, in one paragraph: 'BusOut n' and 'BusIn n' (and
'Out', which writes to a hardware-routed bus the same way 'BusOut'
writes to a plain bus) interact through a *shared resource* — the bus,
identified by an integer. There is no structural 'Audio' edge between
a writer and a reader; they are connected only by the fact that they
name the same bus number. So the structural dependency graph alone
(E_s, what 'dependencies' returns) does not constrain their execution
order. Topological sort over E_s alone could schedule a 'BusIn 5'
before any 'BusOut 5' on the same bus, and the reader would see a
zero-cleared bus instead of the live signal it expects.

The 'Eff' type was designed for exactly this: every node carries an
effect annotation ('BusWrite n', 'BusRead n', 'BusReadDelayed n',
'BufRead n', 'BufWrite n', or 'Pure'), and the *semantically
schedulable* graph is

    G* = (N, E_s ∪ E_r ∪ E_t)

where E_s are structural edges (audio connections), E_r are
resource-induced edges derived from effects, and E_t are temporal /
rate-boundary edges. (E_t is future work; we only deal with E_s and
E_r today.)
...
(e.g. 'BusIn 5' → ... → 'BusOut 5') is rejected by the same cycle
detector that rejects structural cycles.

Same-cycle semantics for 'BusRead'. A 'BusIn n' always reads the live,
accumulated value because it is forced to follow every 'BusOut n' on
the same bus in the same block.

Cross-cycle semantics for 'BusReadDelayed' — feedback. 'BusInDelayed
n' carries 'BusReadDelayed n', and this pass *deliberately* does not
pair 'BusReadDelayed' with 'BusWrite'. The reader is reading the
*previous block's* snapshot of bus n, which by definition is not
modified by any node executing in the current block. Two consequences:
  ...
  1. A 'BusInDelayed n' may sit anywhere in the topological order
     ...
     "cycle" is across blocks, broken by the runtime's swap of the bus
     pool's snapshot and live buffers.

  2. 'BusInDelayed' on a bus with no writer is well-defined: the
     snapshot stays at the zero-initialized state and the read
     produces silence — same as 'BusIn' on an unwritten bus.

The asymmetric treatment of 'BusRead' vs 'BusReadDelayed' is the
central abstraction that lets the same scheduler accept both
same-cycle routing graphs and feedback graphs without runtime graph
rewriting or implicit delays. In SuperCollider terms, 'BusRead' is
'In.ar' and 'BusReadDelayed' is 'InFeedback.ar'.
...
chained buses (a node that reads bus A and writes bus B), recreates
the same dependency analysis at a different layer, and doesn't compose
with region formation or a future scheduler. E_r is the abstraction
the codebase has been building toward since 'Eff' was introduced; this
pass just connects two notes that were sitting next to each other.

See also: Note [Resource effects] in "MetaSonic.Types", Note [Bus
model: SC-style same-cycle audio buses] in "MetaSonic.Bridge.Source",
Note [Effects are per-UGen, not per-kind] in
"MetaSonic.Bridge.Source", and Note [Bus pool double-buffering] in
@tinysynth/rt_graph.cpp@ for the runtime-side ping-pong that realizes
the snapshot semantics.
-}


-- | Verify that every 'NodeID' referenced by a 'Connection' exists in
-- the graph. Returns 'Left' with a diagnostic on the first missing
-- dependency.
--
...
-- structural graph (E_s) merged with the resource-induced edges (E_r)
-- returned by 'busEdges'.
--
...
The sort is a depth-first search with temporary and permanent mark
sets, following Tarjan's algorithm:

  - Temporary marks detect back edges (cycles). If we visit a node
    that is already temporarily marked, we have found a cycle and
    report an error.

  - Permanent marks record fully visited nodes. If we visit a node
    that is already permanently marked, we skip it.

  - On first visit, we temporarily mark the node, recurse into all its
    dependencies, then remove the temporary mark, add a permanent
    mark, and append the node to the accumulator.

The accumulator is built in reverse; the final result is reversed to
produce execution order (dependencies before dependents).

The dependency adjacency map (depMap) is 'effectiveDeps': it merges
the structural edges (E_s, from 'dependencies') with the
resource-induced edges (E_r, from 'busEdges'). The toposort therefore
respects both kinds of dependency uniformly. Cycles in either set — or
cycles that span both — are caught by the same back-edge detection.
...
-- | Check referential integrity and produce a topological execution
-- order in one pass. This is the entry point used by
-- 'MetaSonic.IR.lowerGraph'.
--
